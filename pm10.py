...
new=pd.read_csv('data/pm10Emissions.csv',header=0, index_col=0)
...
new = new.transpose()
new.columns = pd.MultiIndex.from_product([['PM10'], new.columns])
...
merged['PM10'] = merged['PM10'] / merged['POP']
merged['PM10'] = merged['PM10'].apply(lambda x: np.log(x))
...
mod = RandomEffects(merged['PM10'], exog)
re_res = mod.fit()
print(re_res)
mod = PanelOLS(merged['PM10'], exog)
fe_res = mod.fit()
print(fe_res)
