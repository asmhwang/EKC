...
new=pd.read_csv('data/noEmissions.csv',header=0, index_col=0)
...
new = new.transpose()
new.columns = pd.MultiIndex.from_product([['NO'], new.columns])
...
merged['NO'] = merged['NO'] / merged['POP']
merged['NO'] = merged['NO'].apply(lambda x: np.log(x))
...
mod = RandomEffects(merged['NO'], exog)
re_res = mod.fit()
print(re_res)
mod = PanelOLS(merged['NO'], exog)
fe_res = mod.fit()
print(fe_res)
